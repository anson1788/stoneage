	// Md5A.cpp: implementation of the CMd5 class.
//
//////////////////////////////////////////////////////////////////////
...
void MD5Init (MD5_CTX_ENCRYPT *context)    /* context */
{
...
{
  MD5_CTX_ENCRYPT context;
  unsigned char digest[16];
  char output1[34];
  static char output[35]={"\0"};
  ...
  for (i = 0; i < 16; i++)
  {
	  sprintf_s(&(output1[2*i]),34-2*i,"%02x",(unsigned char)digest[i]);
	  //sprintf_s(&(output1[2*i+1]),"%02x",(unsigned char)(digest[i]<<4));
  }
  for(i=0;i<32;i++)
  output[i]=output1[i];
  return output;
...
//		AfxMessageBoxNew("open file error");
//		return "";
//    }
//	else 
//	{
...
//		for (i = 0; i < 16; i++)
//		{
//			sprintf_s(&(output1[2*i]),"%02x",(unsigned char)digest[i]);
//  		//	sprintf_s(&(output1[2*i+1]),"%02x",(unsigned char)(digest[i]<<4));
//  		}
//        for(i=0;i<32;i++)
//			output[i]=output1[i];
...
*/
        ...
        for (i = 0; i < 16; i++)
        {
			sprintf_s(&(output1[2*i]),34-i*2,"%02x",(unsigned char)digest[i]);
			sprintf_s(&(output1[2*i+1]),34-2*i+1,"%02x",(unsigned char)(digest[i]<<4));
        }
        for(i=0;i<32;i++)
			output[i]=output1[i]; 
        return output;     
