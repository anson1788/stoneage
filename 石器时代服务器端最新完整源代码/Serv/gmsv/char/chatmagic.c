    ...
    if( strlen(token)==0){
		...
		for( i=0; i<playernum; i++){
			...
			if( !strcmp( CHAR_getChar( i, CHAR_CDKEY), message) ){
				sprintf(kcmsg,"%s挤出服务器。",CHAR_getChar(i, CHAR_NAME));
...
#endif
				{
					...
					LogCharOut(charname,cdkey,__FILE__,__FUNCTION__,__LINE__,"因做坏事而断线");
#endif
...
{    
    int i=0;
		...
		for( i=0; i<playernum; i++){
			...
			CONNECT_getCharname(CHAR_getWorkInt( i, CHAR_WORKFD ),charname,32);
			CONNECT_getCdkey(CHAR_getWorkInt( i, CHAR_WORKFD ),cdkey,16);
			LogCharOut(charname,cdkey,__FILE__,__FUNCTION__,__LINE__,"CHAR_CHAT_DEBUG_waeikickall函数处理");
#endif
			...
			for( object = MAP_getTopObj(floor,j,i) ; object ;object = NEXT_OBJECT(object ) ){
				...
				if( OBJECT_getType(o) == OBJTYPE_CHARA ){
					...
					if( chara_index == charaindex ) continue;  //找到自己了
						getnum++;
						snprintf( line, sizeof( line), "第%d个找到[%s]的帐号是[%s]",getnum,CHAR_getChar(chara_index, CHAR_NAME),CHAR_getChar(chara_index, CHAR_CDKEY));
						...
						while(object){
							...
							if( OBJECT_getType(o) == OBJTYPE_CHARA ){
								...
								if( CHAR_getInt( chara_index, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER ){
										...
										snprintf(line,sizeof( line),"第%d个传送[%s]",sendnum,CHAR_getChar(chara_index, CHAR_NAME));
...
void CHAR_CHAT_DEBUG_PlayerQuestion( int charaindex, char* message )
{
	...
	if(getStringFromIndexWithDelim(message," ",3, tmp, sizeof(tmp))== FALSE){
		CHAR_talkToCli( charaindex, -1, "请输入你的奖品类型(0为石币,1为道具,2为宠物,3为声望,4为积分!",  CHAR_COLORRED);
	}else{
	...
	
	{
	  ...
	  sprintf(question, "[GM]在线问题——%s", PlayerQuestion.question);
		sprintf(token, "第一位回答正确者奖励%s %d", type[PlayerQuestion.type], PlayerQuestion.value);
...
void CHAR_CHAT_DEBUG_petmake2( int charaindex, char* message )
{
	...
	{
		CHAR_talkToCli(charaindex, -1, "参数不正确 [petmaketrans 转生数 宠物ID 等级 数量]", CHAR_COLORRED);
		return;
	}
	nTrans = atoi(szBuf1);
	...
	if (nMakeNum == 0)	// 正常的产生宠物
	{
		...
		if (nPetNo == -1)
		{
			CHAR_talkToCli(charaindex, -1, "制作好宠物，但在人物身上找不到", CHAR_COLORRED);
			return;
		}
		...
		for (j = 0; j < nTrans; j++)
		{
			...
			sprintf(szMsgbuf, "%d 转 %d 前：Vital:%d Str:%d Tgh:%d Dex:%d",
			...
			nLevelUpPoint = CHAR_getInt(nPetIndex, CHAR_ALLOCPOINT);
			sprintf(szMsgbuf, "%d 转 %d 后：Vital:%d Str:%d Tgh:%d Dex:%d",
				CHAR_getInt(nPetIndex, CHAR_TRANSMIGRATION),
				CHAR_getInt(nPetIndex, CHAR_TRANSMIGRATION) + 1,
	...
	else if (nMakeNum > 0 && nMakeNum < MAX_MAKE_PET_NUM)	// 大量生产宠物
	{
		...
		snprintf(szMsgbuf, sizeof(szMsgbuf), "测试转宠制作成功\!(%d只)", nMakeCount);
		CHAR_talkToCli(charaindex, -1, szMsgbuf, CHAR_COLORWHITE);
	}
}
#endif
	...
	if(strstr( message , "check" ) != NULL )
	{
		for( i=0;i<BANKERTOTAL;i++ )
		{
			if( tickettotal[i] == 0 || Bettotal[i] == 0 || Wintotal[i] == 0)
				sprintf(buf,"庄家%d 无任何纪录",i);
			else
				sprintf(buf,"庄家%d  bettotal=%d wintotal=%d  rate=%d  tickets=%d",i,Bettotal[i],Wintotal[i],Wintotal[i]*100/Bettotal[i],tickettotal[i]);
			CHAR_talkToCli( charaindex, -1, buf , CHAR_COLORYELLOW);
		}
	}
