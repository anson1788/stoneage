...
#ifdef _COMPOUND_EXP
char mergech[][16] = {"石", "木", "骨", "牙", "皮", "贝壳" , "壳", "爪", "花", "叶", "线", "黏土", "鳞", "毒", "石化", "混乱", "酒醉", "睡眠", "回复", "复活", "水晶", "地", "水", "火", "风"};
#endif
			...
			for( i = 0 ; i < 5; i++ ) {
				...
				if( created >= 0 ) {
					...
					if (ITEMTBL_getInt( created, ITEM_LEVEL)>= getMaxMergeLevel()){
						CHAR_talkToCli( charaindex, -1 ,"无法合成超越设定的装备哦。", CHAR_COLORRED);
						return items[RAND( 0, (num-1))].data[ITEM_ID];
					}else{
						return created;
					}
...
#define MAXCANINSLAY 3
	...
	if( i == MAXCANINSLAY )	{
		char token[256];
		sprintf( token, "武器或防具已经镶满%d颗或%d颗以上。", MAXCANINSLAY, MAXCANINSLAY);
