...
int BATTLE_CreateVsPlayer( int charaindex0, int charaindex1 )
{
	...
	if( CHAR_getWorkInt( charaindex0, CHAR_WORKDBATTLETIME ) + 120 > time(NULL)){
		char token[256];
		sprintf(token, "你当前需冷却%d后才能进行PK", CHAR_getWorkInt( charaindex0, CHAR_WORKDBATTLETIME ) + 120 - time(NULL));
		CHAR_talkToCli( charaindex0, -1, token, CHAR_COLORYELLOW );
		return BATTLE_ERR_ALREADYBATTLE;
	}
	if( CHAR_getWorkInt( charaindex1, CHAR_WORKDBATTLETIME ) + 120 > time(NULL)){
		char token[256];
		sprintf(token, "对方需冷却%d后才能进行PK", CHAR_getWorkInt( charaindex1, CHAR_WORKDBATTLETIME ) + 120 - time(NULL));
		CHAR_talkToCli( charaindex0, -1, token, CHAR_COLORYELLOW );
		sprintf(token, "你当前需冷却%d后才能进行PK", CHAR_getWorkInt( charaindex1, CHAR_WORKDBATTLETIME ) + 120 - time(NULL));
			...
			if( CHAR_getInt( charaindex, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER ){
				if(CHAR_getWorkInt(charaindex, CHAR_BATTLE_BOUT_TIME) > (int)time(NULL) - 1){
					if(CHAR_getInt(charaindex, CHAR_MP)>0){
						CHAR_talkToCli( charaindex, -1, "系统监测到您正在快速战斗，把您的MP自动清0", CHAR_COLORYELLOW);
...
)
{
	...
	if(floor == 11111 || floor == 22222 || floor == 33333 || floor == 44444){
		...
		if( floor == 20000 && AutoPk_PKTimeGet() <= 0){
				if( dpadd == 0 ){
					CHAR_talkToCli(charaindex, -1, "战绩不理想，未加减竟赛积分！", CHAR_COLORRED);
				}else	if( dpadd < 0 ){
					int fl = 0, x = 0, y = 0;
					if(CHAR_CHECKINDEX(charaindex) != FALSE){
...
//					  saacproto_FormulateAutoPk_send(acfd, CHAR_getChar(charaindex, CHAR_CDKEY), CHAR_getWorkInt(charaindex,CHAR_WORK_AUTOPK));
					  ...
					  CHAR_talkToCli(charaindex, -1, "你已战败，系统自动把你送回记录点", CHAR_COLORYELLOW);
	...
	if( CHAR_getInt( enemyindex, CHAR_WHICHTYPE ) == CHAR_TYPEPLAYER 
		...
		if( BattleArray[battleindex].type == BATTLE_TYPE_P_vs_P ){
			...
			//			BATTLE_Exit( badindex, battleindex );
						...
						*/
						char message[256];
						sprintf(message, "%s 被 %s 击飞至远方， %s 因飞人（宠），二人恩怨值火爆上升！！", CHAR_getUseName( enemyindex ), CHAR_getUseName( badindex ), CHAR_getUseName( badindex ));
					  int i;
					  int playernum = CHAR_getPlayerMaxNum();
					  for( i = 0 ; i < playernum ; i++) {
...
//*************************************************************
{
	...
	&&  BattleArray[battleindex].norisk == 0
			...
			//			BATTLE_Exit( badindex, battleindex );
						...
						*/
						char message[256];
						sprintf(message, "%s 被 %s 击飞至远方， %s 因飞人，二人恩怨值火爆上升！！", CHAR_getUseName( enemyindex ), CHAR_getUseName( badindex ), CHAR_getUseName( badindex ));
					  int i;
					  int playernum = CHAR_getPlayerMaxNum();
					  for( i = 0 ; i < playernum ; i++) {
					    if( CHAR_getCharUse(i) != FALSE ) {
...
static int BATTLE_Battling( int battleindex )
{
		...
		case BATTLE_COM_S_PETOUT:
			if( COM == BATTLE_COM_S_PETOUT ){
				...
				if( CHAR_getInt( defindex, CHAR_WHICHTYPE ) == CHAR_TYPEPLAYER ){
					...
					CHAR_talkToCli( charaindex, -1 , "该技能只能对玩家人物使用!" , CHAR_COLORYELLOW );
				}
				break;
			}
#endif
	...
	if(COM == BATTLE_COM_S_INVERSION ){
		if((Battle_Attack_ReturnData_x.Battle_Attack_ReturnData != BATTLE_RET_MISS)  //没有miss
			 && (Battle_Attack_ReturnData_x.Battle_Attack_ReturnData != BATTLE_RET_DODGE)
			 && (Battle_Attack_ReturnData_x.Battle_Attack_ReturnData != BATTLE_RET_ALLGUARD) //没有闪避过
...
/*
#ifndef _VERSION_25 // Robin fix 战斗不可换防具饰品
			{
