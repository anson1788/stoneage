...
void AutoPk_ChampionShipSet( int charaindex, int winnum, int win )
{
	...
	if(win==0){
		...
		if(winnum>ship[0].winnum){
			sprintf( token, "%s排名第一，杀人得分总数%d分！", name, winnum);
			...
			sprintf( token, "%s排名第二，杀人得分总数%d分！", name, winnum);
			AutoPk_PKSystemTalk(token,token);
			...
			sprintf( token, "%s排名第三，杀人得分总数%d分！", name, winnum);
			AutoPk_PKSystemTalk(token,token);
...
void AutoPk_GetChampionShip( void )
{
	...
	if(strlen(ship[0].cdkey)>0){
		sprintf( token, "恭喜%s获得本次PK比赛冠军，杀人得分总数%d分！", ship[0].name, ship[0].winnum);
		AutoPk_PKSystemTalk(token,token);
	}
	if(strlen(ship[1].cdkey)>0){
		sprintf( token, "恭喜%s获得本次PK比赛亚军，杀人得分总数%d分！", ship[1].name, ship[1].winnum);
		AutoPk_PKSystemTalk(token,token);
	}
	if(strlen(ship[2].cdkey)>0){
		sprintf( token, "恭喜%s获得本次PK比赛季军，杀人得分总数%d分！", ship[2].name, ship[2].winnum);
		AutoPk_PKSystemTalk(token,token);
	}
