...
//				print("\n %s %dø™∑≈",CHAR_getChar(talker,CHAR_NAME),(int)PLAYER.ptr);
				...
				//ÚÂªØº∞±Œèm÷ ÜAª•±Œ¥ı‘»ªØæÆ»’∑•°ı√ÛΩÒ¡˘‘¬©z
				if(warp_flg != -1){
			...
			/*--≥º∞???±Â…¨¿√ΩÒƒæªØ÷–‘¬Õ÷ˆº∞ñÑÀ¯?ƒØ√´?ŒÂ«Î‘ --*/
			tbl =  NPC_GetQuestion( meindex);
			
			
			/*--ñÑÀ¯–—º∞√Òƒ·”¿€Õ tbl[0]±Â∑¥Õ÷ˆ√´àm–◊ÿ∆–◊ñÑÀ¯–—ª•∑¥÷–‘»ªØ÷–‘¬ --*/
			if( CHAR_getWorkInt( meindex, CHAR_WORK_QUIZNUM) > ( tbl[0] - 1))
			{
				/*--ñÑÀ¯ª•ÛÔ‘ªÿ¶÷–--*/
				/*--∂™∆πÿ¯ê_‡¢∑∞ïC--*/
			...
			/*--€ÕƒÃ ıº∞ñÑÀ¯º∞”ÆΩÒ±Â∑Ω‘»ªØ©yºª«πÑ∆º∞Ñ\‘˜ﬁÀŒ˚√´éﬂ“¸‘¬--*/
			if(strlen( buf) < 40) {
				strcat( buf, "\n");
...
BOOL NPC_QuizAddItem(int talker,char *buf)
{
	...
	/*--ÖÕ–—≤ŸàVŒÁÿ∆ªØ ßƒÃ∑∫ÿ©ª•…¨¿√ΩÒƒæªØ÷–‘¬ËÎƒ˛∑¥∑¬º˛ƒ∏ÿ©∆•ºª…Ÿ--*/
	...
	/* ßƒÃ∑∫ÿ©º∞‹∞µ—(€QÃŒ ßƒÃ∑∫ÿ©Ñ∆±Â÷–ƒæªØÿ∆“˝µ§©k*/
...
BOOL NPC_PlayerCheck(int meindex,int talker)
	...
	Player.quizno = 0;		//Úÿª˛º∞ñÑÀ¯–—
	Player.answer = 0;		//ø“›©–—
	Player.ansno = 0;		//≤œ“¸º∞?ƒØ√´¥¥“¸ªØ‘∆» ©j≤œ“¸√´ÃÔ∑¬ÃÔ∑¬±Â«Îÿ∆–◊∂À±Â“¯Â√)
