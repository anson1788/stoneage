...
enum {
	...
	,NPC_WORK_SUPERFMPOINT = CHAR_NPCWORKINT7	// 记录
#endif
};
...
#ifdef _NEW_MANOR_LAW
								sprintf(buf, "你的家族气势尚未到达 3000 ，请再加油。");
														//	sprintf(buf, "你的家族气势尚未到达庄园家族气势的九成，请再加油。");
#else
	#ifdef _MANOR_PKRULE
		#ifdef _FIX_FAMILY_PK_LIMIT	   // WON ADD 将可挑战庄园的由前十大改为前三十大
                sprintf(buf, "你的家族声望尚未进入前三十大家族，请再加油。");
		#else
...
/*
    ...
    lssproto_WN_send( fd, WINDOW_MESSAGETYPE_MESSAGE,
		...
		CHAR_talkToCli(talkerindex,-1,"扣除个人声望500点",CHAR_COLORYELLOW);
	}
	*/
#endif
