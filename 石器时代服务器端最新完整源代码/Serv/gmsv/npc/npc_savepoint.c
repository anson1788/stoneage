	    ...
	    // Robin 0619 display time
	    {
	        ...
	        ss=(int) new_t;
	        	...
	        	snprintf( timeMsg, sizeof( timeMsg ) ,
        			  "本星球%s已经启动了 %d 日 %d 小时 %d 分 %d 秒。",getGameservername(),dd,hh,mm,ss);
                } else if (hh>0) {
 	                snprintf( timeMsg, sizeof( timeMsg ) ,
      		                  "本星球%s已经启动了 %d 小时 %d 分 %d 秒。",getGameservername(),hh,mm,ss);
                } else {
                        snprintf( timeMsg, sizeof( timeMsg ) ,
                                  "本星球%s已经启动了 %d 分 %d 秒。",getGameservername(),mm,ss);
                }
                strcat(buf2,timeMsg);
#else
					if (dd>0) {
...
#ifdef _OFFLINE_SYSTEM
					if(getPlayerNum()>0){
						sprintf(temp,"目前线上人数为 %d 人。", playeronlinenum + luaplayernum + getPlayerNum());
					}else{
